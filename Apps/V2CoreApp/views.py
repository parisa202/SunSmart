...
class CustomLoginRequiredMixin:
    def dispatch(self, request, *args, **kwargs):
        if 'user_id' not in request.session:
            ...
            return redirect('V2CoreApp:login_or_register')
        return super().dispatch(request, *args, **kwargs)
    
    
class HomeView(generic.TemplateView):
    template_name = 'V2CoreApp/index.html'
    
    def import_data(self):
        filename = "C:/Users/Parisa/Downloads/tb_concern.csv"
        df = pd.read_csv(filename)
        for index, row in df.iterrows():
            temp_row = PB_AU_CONCERN_V2(concern=row['concern'], 
                                        gender=row['gender'],
    ...
    def import_bmi_data(self):
        ...
        df = pd.read_csv(filename)
        for index, row in df.iterrows():
            temp_row = PB_WHO_BMI_V2(bmi=row['bmi_code'], 
                                        gender=row['gender'],
                                        year=row['year'],
...
class US11View(generic.TemplateView):
    template_name = 'V2CoreApp/overweight.html'
    
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        super().get_context_data(**kwargs)
        
        # query from table
        all_data = PB_AU_CONCERN_V2.objects.all()
        serialized_data = serializers.serialize('json', all_data)
        
        
        all_data_bmi = PB_WHO_BMI_V2.objects.all()
...
class US11DashboardView(generic.TemplateView):
    template_name = 'V2CoreApp/us11_dashboard.html'
...
class US121View(generic.TemplateView):
    template_name = 'V2CoreApp/us121.html'


class US13View(generic.TemplateView):
    template_name = 'V2CoreApp/additive.html'
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        super().get_context_data(**kwargs)
...
class US21View(generic.TemplateView):
    template_name = 'V2CoreApp/nutrition-guideline.html'
    data = ""
...
class US22View(generic.TemplateView):
    template_name = 'V2CoreApp/us22.html'

#Reminder page
class ReminderView(generic.TemplateView):
    template_name = 'V2CoreApp/reminder_main.html'
    ...
    def post(self, request):
        ...
        return redirect('V2CoreApp:US23')
...
class GuestTrackerView(generic.TemplateView):
    ...
    def post(self, request):
        ...
        # Not AJAX request
            ...
            return redirect('V2CoreApp:guest_tracker')
...
class MacronutrientsView(generic.TemplateView):
    template_name = 'V2CoreApp/Macronutrients.html'
    macro_info = None
...
class MicronutrientsView(generic.TemplateView):
    template_name = 'V2CoreApp/Micronutrients.html'
    micro_info = None
...
class RecipeDetailView(generic.DetailView):
    model = RECIPES_V2
    template_name = 'V2CoreApp/recipe_detail.html'
    slug_field = 'slug'
...
class RecipeListView(generic.ListView):
    template_name = 'V2CoreApp/recipe_list.html'
    model = RECIPES_V2
...
class OuterRecipeAnalysisView(generic.TemplateView):
    template_name = 'V2CoreApp/outer_recipe_analysis.html'
    headers = {
        "Content-Type": "application/json"
    }
...
class ComingView(generic.TemplateView):
    template_name = 'V2CoreApp/coming_soon.html'
...
class ImportDataView(generic.View):
    def get(self, request, *args, **kwargs):
        ...
        model_class =  apps.get_model('V2CoreApp', model_name)
...
class VeggiView(generic.TemplateView):
    template_name = 'V2CoreApp/vegetable.html'
    ...
    def get_context_data(self, **kwargs: Any) -> dict[str, Any]:
        super().get_context_data(**kwargs)
         ...
         # query from table
        all_data = PB_AU_VEGGIE_V2.objects.all()
        serialized_data = serializers.serialize('json', all_data)
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            with open(file_address, 'r') as f:
                ...
                for r in recipes:
                    # create RECIPE
                    temp_recipe, created = RECIPES_V2.objects.get_or_create(
                                                    name=r["recipe"]["label"],
                    ...
                    if created:
                        ...
                        for tag in r["recipe"]["healthLabels"]:
                            temp_health_tag, c = HEALTH_TAG_V2.objects.get_or_create(tag=tag)
                            # adding Healthe tags
                            temp_recipe.health_tag.add(temp_health_tag)
                        
                        # MEAL TYPE   
                        for type in r["recipe"]["mealType"]:
                            temp_meal_type, c = MEAL_TYPE_V2.objects.get_or_create(type=type)
                            # adding meal type
                            temp_recipe.meal.add(temp_meal_type)
                    
                        # DIET LABEL   
                        for label in r["recipe"]["dietLabels"]:
                            temp_diet_label, c = DIET_LABEL_V2.objects.get_or_create(label=label)
                            # adding diet label
                            temp_recipe.diet_label.add(temp_diet_label)
                                                             
                        # INGREDIENTS
                        for ingredient in r["recipe"]["ingredients"]:
                            temp_ingredient, c = INGREDIENT_V2.objects.get_or_create(name=ingredient['food'], food_category=ingredient['foodCategory'])   
 
                            # creating RECIPE_INGREDIENT
                            RECIPE_INGREDIENT_V2.objects.create(
                                                        recipe = temp_recipe,
                        ...
                        for key, nutrient in r["recipe"]["totalNutrients"].items():
                            temp_nutrient, c = NUTRIENTS_V2.objects.get_or_create(name=nutrient['label'])   
 
                            # creating RECIPE_NUTRIENT
                            RECIPE_NUTRIENTS_V2.objects.create(
                                                        recipe = temp_recipe,
